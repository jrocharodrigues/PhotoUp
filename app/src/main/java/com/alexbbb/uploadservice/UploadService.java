...
public class UploadService extends IntentService {
        ...
        for (FileToUpload file : filesToUpload) {
            ...
            try {
                while ((bytesRead = stream.read(buffer, 0, buffer.length)) > 0) {
                    ...
                    broadcastProgress(uploadId, uploadedBytes, totalBytes, currentItem);
                }
            } finally {
    ...
    private void broadcastProgress(final String uploadId, final long uploadedBytes, final long totalBytes, final int currentItem) {
